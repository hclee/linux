 ...
 * @al_cursor: Restartable attr-list enumeration cursor.  al_cursor.valid is
 *	also used as "did ntfs_external_attr_find() run for this ctx's
 *	current attribute": ntfs_external_attr_find() unconditionally makes
 *	al_cursor.valid true (freshly, or by leaving an already-valid cursor
 *	alone) before any return path, with no early return in between, so
 *	al_cursor.valid is true exactly when a lookup went through it.
